...
public class ClassReader {
     ...
     */
    private final int[] items;//常量池中各种项目的起始偏移量(常量类型标识的后一位为起始位)，常量顺序与常量起始偏移量的对应关系
     ...
     */
    private final String[] strings;//缓存CONSTANT_Utf8_info类型的常量值，索引为CONSTANT_Utf8_info类型的常量在常量池中的顺序位置

    /**
     * Maximum length of the strings contained in the constant pool of the
     * class.
     */
    private final int maxStringLength; //CONSTANT_Utf8_info类型的常量中占用字节数最多的常量大小

    /**
     * Start index of the class header information (access, name...) in
     * {@link #b b}.
     */
    public final int header; //header为类访问标识符(access_flags)的偏移量
    ...
    public ClassReader(final byte[] b, final int off, final int len) {
    	//class文件的字节码数组
        this.b = b;
        // checks the class version检查class文件版本
		/* SPRING PATCH: REMOVED FOR FORWARD COMPATIBILITY WITH JDK 9
        if (readShort(off + 6) > Opcodes.V1_8) {//获取class文件版本
            throw new IllegalArgumentException();
        }
		*/
        // parses the constant pool解析常量池
        items = new int[readUnsignedShort(off + 8)];
        int n = items.length;
        strings = new String[n];
        int max = 0; //记录最大CONSTANT_Utf8_info常量的所占用的字节数
        int index = off + 10; //常量池中第一个常量起始偏移量
        for (int i = 1; i < n; ++i) {//常量是从1开始计数
            items[i] = index + 1; //常量池中各项常量的起始偏移量(常量类型标识符的后一位)
            int size; //常量池中的项目占字节数
            switch (b[index]) {//常量池项目类型标识
            case ClassWriter.FIELD: //说明该常量是CONSTANT_Fieldref_info
            case ClassWriter.METH:  //说明该常量是CONSTANT_Methodref_info
            case ClassWriter.IMETH: //CONSTANT_InterfaceMethodref_info
            case ClassWriter.INT:   //CONSTANT_Integer_info
            case ClassWriter.FLOAT: //CONSTANT_Float_info
            case ClassWriter.NAME_TYPE: //CONSTANT_NameAndType_info
            case ClassWriter.INDY:      //CONSTANT_InvokeDynamic_info
                size = 5;
                break;
            case ClassWriter.LONG: //CONSTANT_Long_info
            case ClassWriter.DOUBLE: //CONSTANT_Double_info
                size = 9;
                ++i;
                break;
            case ClassWriter.UTF8: //CONSTANT_Utf8_info
                size = 3 + readUnsignedShort(index + 1);
                if (size > max) {
                    max = size;
                }
                break;
            case ClassWriter.HANDLE: //CONSTANT_MethodHandle_info
                size = 4;
                break;
        ...
        maxStringLength = max;//CONTANT_Utf8_info类型的常量中占用字节最多的常量大小
        // the class header information starts just after the constant pool
        header = index;//header为类访问标识符(access_flags)的偏移量
    }
    ...
    public void accept(final ClassVisitor classVisitor,
    	...
    	//header表示类访问修饰符符(access_flags)在class文件中的起始偏移量
        int u = header; // current offset in the class file
        //maxStringLength是class文件中占用字节数最多CONSTANT_Utf8_info的长度
        ...
        int access = readUnsignedShort(u);//获取类访问修饰符(access_flags)
        String name = readClass(u + 2, c);//获取类的全限定名称
        String superClass = readClass(u + 4, c);//获取超类的全限定名称
        //readUnsignedShort(u + 6) : 获取类实现接口的数量
        String[] interfaces = new String[readUnsignedShort(u + 6)];
        u += 8;//如果类有实现接口，则为接口表的起始偏移量
        for (int i = 0; i < interfaces.length; ++i) {
            interfaces[i] = readClass(u, c);//实现的接口的全限定名
            u += 2;
        }
        ...
        u = getAttributes();//获取类属性数量起始偏移量
        //readUnsignedShort(u)： 获取类属性数量
        for (int i = readUnsignedShort(u); i > 0; --i) {
            String attrName = readUTF8(u + 2, c);//获取类属性名称
            // tests are sorted in decreasing frequency order
            // (based on frequencies observed on typical classes)
            if ("SourceFile".equals(attrName)) {
                sourceFile = readUTF8(u + 8, c);//获取SourceFile属性的值
            } else if ("InnerClasses".equals(attrName)) {
                innerClasses = u + 8; //内部类的起始偏移量
    ...
    private int getAttributes() {
        // skips the header
    	/**
    	 * header + 8 ：如果存在接口索引集合的话，则是接口索引集合的起始偏移量 
    	 * header + 8 + readUnsignedShort(header + 6) * 2 ：字段表数量起始偏移量
    	 */
        int u = header + 8 + readUnsignedShort(header + 6) * 2;//u为字段表数量起始偏移量
        // skips fields and methods
        for (int i = readUnsignedShort(u); i > 0; --i) {
        	/**
        	 * 计算每个属性占用多少字节
        	 * u + 8 ： 字段属性数量(atrributes_count)起始偏移量
        	 */
            for (int j = readUnsignedShort(u + 8); j > 0; --j) {//如果字段有属性
            	/*
            	 * u + 12: 每个字段属性值长度(attribute_length)起始偏移量
            	 * readInt(u+12) : 获取属性值占多少个字节
            	 */
                u += 6 + readInt(u + 12);//每个属性的长度 = 6 + 属性值所占字节数
            }
            u += 8;//field of access_flag , name_index, descriptor_index , attribute_count
        }
        u += 2; //field_count of 2-bytes
        /*
         * readUnsignedShort(u) : 获取方法表集合大小
         */
        for (int i = readUnsignedShort(u); i > 0; --i) {
        	/**
        	 * u + 8 : 方法属性数量的起始偏移量
            ...
            for (int j = readUnsignedShort(u + 8); j > 0; --j) {
            	/**
            	 * u + 12 : 方法属性值长度(attribute_length)起始偏移量 
            	 */
                u += 6 + readInt(u + 12); //每个属性的长度 = 6 + 属性值所占字节数
            }
            u += 8;// method of access_flag, name_index, descriptor_index, attribute_count
        }
        // the attribute_info structure starts just after the methods
        return u + 2; //类属性数量起始偏移量
    }
     ...
     * @return the read value.
     * 1.获取常量池大小
     * 2.获取CONSTANT_Utf8_info常量值所占用的字节数
     * 3.获取类索引(this_class)，该索引指向常量池中的一个类型为CONSTANT_Class_info的常量
     */
    public int readUnsignedShort(final int index) {
        byte[] b = this.b;
        /**
         * b[8] : 0000 0000
         * 与0xFF(即1111 1111)进行与运算： 0000 0000
         * 向左位移8位： 0000 0000 0000 0000
         * 
         * b[8 + 1] : 15(即二进制表示 0001 0101)
         * 与0xFF进行与运算： 0001 0101
         * 
         * 将左右两位进行或运算 0000 0000 0001 0101
         */
    ...
    public short readShort(final int index) {
         ...
         * 次版本号：
         * b[6] : 0000 0000 
         *     0xFF : 255 (即二进制 1111 1111)
         * 进行与等于 0000 0000 
         * 左位移8 ： 0000 0000 0000 0000
         * 
         * 主版本号：
         * b[6 + 1] : 0011 0001
         * 与 0xFF进行与运算： 0011 0001
         * 
         * 次版本与主版本或运算： 0000 0000 0011 0001
         */
    ...
    public String readUTF8(int index, final char[] buf) {
    	/**
    	 * 1.从CONSTANT_Class_info中获取指向全限定名常量项的索引
    	 */
        ...
        /**
         * 1.获取CONSTANT_Utf8_info常量起始索引，注意：不包含常量项的类型标识
         */
        index = items[item]; 
        //strings缓存CONSTANT_Utf8_info常量值
     ...
     *            CONSTANT_Utf8_info常量值的起始索引位置
     * @param utfLen
     *            length of the UTF8 string to be read.
     *            CONSTANT_Utf8_info常量值的长度，即常量值占用多少个字节位数
     * @param buf
    ...
    private String readUTF(int index, final int utfLen, final char[] buf) {
        ...
        while (index < endIndex) {
            c = b[index++]; //获取字节码数组中的一个字节
            switch (st) {
            case 0:
                c = c & 0xFF;   //           1111 1111
                /**
                 * 从'\u0001'到'\u007f'之间的字符的缩略码使用一个字节表示，
                 * 其中\\u表示是个unicode字符，之后的数字0001和007f是十六进制表示,在这里分别表示1和127
                 * 当读取到第一位时，发现是0开头，说明该字符占用1个字节
                 */
                if (c < 0x80) { // 0xxxxxxx  1000 0000
                    buf[strLen++] = (char) c;
                 /**
                  * E0: 224(即二进制表示1110 0000) ，BF: 191(即二进制表示1011 1111)
                  * 解释：比E0(即1110 0000小的数肯定是110开头的数，比BF(即1011 1111)大的数肯定也是以110开头的数，
                  * 同时，比E0小的最大数是1101 1111 ，比BF大的最小数是1100 0000。
                  * 当读取到第一位时，发现是110开头，说明该字符占用两个字节
                  */
                } else if (c < 0xE0 && c > 0xBF) { // 110x xxxx 10xx xxxx 
                    cc = (char) (c & 0x1F);  //1F: 二进制表示0001 1111
                    st = 1;
                    /**
                     * 否则说明该字符占用3个字节
                     */
                } else { // 1110 xxxx 10xx xxxx 10xx xxxx
                    cc = (char) (c & 0x0F); //0F: 二进制表示0000 1111  
                    st = 2;
                }
                break;

            case 1: // byte 2 of 2-byte char or byte 3 of 3-byte char
                buf[strLen++] = (char) ((cc << 6) | (c & 0x3F)); //3F:即二进制表示 0011 1111
                st = 0;
                break;
    ...
    private String readStringish(final int index, final char[] buf) {
    	...
    	/**
    	 * items常量池中各项常量的起始偏移量，即常量顺序与常量起始偏移量(不包含常量类型标识位)之间的对应关系
    	 * readUnsignedShort(index) ： 返回指向常量池中常量项的索引
    	 * items[readUnsignedShort(index) ： 返回常量项起始偏移量(不包含常量类型标识位)
    	 */
